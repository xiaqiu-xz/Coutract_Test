...
void init() { pactffi_init("LOG_LEVEL"); }
////////////////////////////////////
// Pact Class
////////////////////////////////////
...
Interaction Pact::given(const char* provider_state,
    ...
    if (mockServer.started_ok()) {
        try {
            ...
            if (callback_result && mock_server_result) {
                auto write_result = pactffi_write_pact_file(mockServer.get_port(),
                                                            this->pact_directory.data(), false);
                switch (write_result) {
                    case 1:
                        result.add_state(TestResultState::PactFileError,
                                         "A general panic was caught");
                        break;
                    case 2:
                        result.add_state(TestResultState::PactFileError,
                                         "The pact file was not able to be written");
                        break;
                    case 3:
                        result.add_state(TestResultState::PactFileError,
...
Interaction Interaction::given(
    ...
    for (auto& p : parameters) {
        pactffi_given_with_param(this->interaction, provider_state, p.first.data(),
                                 p.second.data());
    }
    return *this;
}
...
Interaction Interaction::withQuery(
    const std::unordered_map<std::string, std::vector<std::string>>& query) const {
    for (auto& q : query) {
        for (auto it = q.second.begin(); it != q.second.end(); it++) {
            pactffi_with_query_parameter(this->interaction, q.first.data(), it - q.second.begin(),
                                         it->data());
        }
    }
    return *this;
}
Interaction Interaction::withHeaders(
    const std::unordered_map<std::string, std::vector<std::string>>& headers) const {
    for (auto& h : headers) {
        for (auto it = h.second.begin(); it != h.second.end(); it++) {
            pactffi_with_header(this->interaction, InteractionPart_Request, h.first.data(),
                                it - h.second.begin(), it->data());
        }
    }
    return *this;
}
...
Interaction Interaction::withBinaryFile(const std::string& content_type,
                                        const std::filesystem::path& example_file) const {
...
Interaction Interaction::withMultipartFileUpload(const std::string& part_name,
                                                 const std::string& content_type,
                                                 const std::filesystem::path& example_file) const {
    auto result =
        pactffi_with_multipart_file(this->interaction, InteractionPart_Request, content_type.data(),
...
Interaction Interaction::withResponseHeaders(
    const std::unordered_map<std::string, std::vector<std::string>>& headers) const {
    for (auto h : headers) {
        for (auto it = h.second.begin(); it != h.second.end(); it++) {
            pactffi_with_header(this->interaction, InteractionPart_Response, h.first.data(),
                                it - h.second.begin(), it->data());
        }
    }
    return *this;
}
Interaction Interaction::withResponseBody(const std::string& body,
...
Interaction Interaction::withResponseBinaryFile(const std::string& content_type,
                                                const std::filesystem::path& example_file) const {
    std::ifstream file(example_file, std::ios::binary | std::ios::ate);
    std::streamsize size = file.tellg();
    file.seekg(0, std::ios::beg);
...
Interaction Interaction::withResponseMultipartFileUpload(
    const std::string& part_name, const std::string& content_type,
    const std::filesystem::path& example_file) const {
    auto result = pactffi_with_multipart_file(this->interaction, InteractionPart_Response,
                                              content_type.data(), example_file.string().data(),
                                              part_name.data());
    if (result.tag == StringResult_Failed) {
        std::string error = result.failed;
        pactffi_free_string(result.failed);
...
MockServerHandle::MockServerHandle(PactHandle pact) {
    ...
    this->port = pactffi_create_mock_server_for_transport(pact, "127.0.0.1", 0, "http", NULL);
}
MockServerHandle::~MockServerHandle() { pactffi_cleanup_mock_server(this->port); }
bool MockServerHandle::started_ok() const { return this->port > 0; }
std::string MockServerHandle::get_url() const {
    std::ostringstream out;
    out << "http://127.0.0.1:" << this->port;
    return out.str();
}
int32_t MockServerHandle::get_port() const { return this->port; }
////////////////////////////////////
// Pact Test Result Class
////////////////////////////////////
PactTestResult::PactTestResult() { this->messages.resize(TestResultState::MockServerFailed + 1); }
void PactTestResult::add_state(TestResultState state) { this->status |= state; }
...
bool PactTestResult::is_ok() const { return this->status == 0; }
            ...
            if (this->messages[TestResultState::Mismatches].empty()) {
                ...
                for (json::iterator it = j.begin(); it != j.end(); ++it, i++) {
                    ...
                    if (type == "request-not-found") {
                        std::cout << "    " << i
                                  << ") An unexpected request was received: " << mismatch["method"]
                                  << " " << mismatch["path"] << "\n\n";
                    } else if (type == "missing-request") {
                        std::cout << "    " << i << ") An expected request was not received: "
                                  << mismatch["method"] << " " << mismatch["path"] << "\n\n";
                    } else if (type == "request-mismatch") {
                        std::cout << "    " << i
                                  << ") Mismatched request was received: " << mismatch["method"]
                                  << " " << mismatch["path"] << "\n\n";
                        auto mismatches = mismatch["mismatches"];
                        int m = 1;
                        for (json::iterator it = mismatches.begin(); it != mismatches.end();
                             ++it, m++) {
                            ...
                            if (mismatch_type == "MethodMismatch") {
                                ...
                                std::string expected = mismatch_details["expected"];
                                std::string actual = mismatch_details["actual"];
                                std::cout << "      " << i << "." << m << ") Body Type: Expected "
                                          << expected << " but was " << actual << "\n";
                            } else if (mismatch_type == "BodyMismatch") {
                                std::string path = mismatch_details["path"];
                                std::string mismatch = mismatch_details["mismatch"];
                                std::cout << "      " << i << "." << m << ") Body: " << path
                                          << " - " << mismatch << "\n";
                            } else {
                                std::cout
                                    << "      " << i << "." << m
                                    << ") An unknown type of mismatch occurred: " << mismatch_type
                                    << "\n";
                            }
                        }
                    }
