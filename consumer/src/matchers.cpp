...
IMatcher::Ptr Integer(long value) { return std::make_shared<IntegerMatcher>(value); }
IMatcher::Ptr Integer(int value) { return std::make_shared<IntegerMatcher>(value); }
IMatcher::Ptr Integer() { return std::make_shared<IntegerMatcher>(); }
IMatcher::Ptr Like(long value) { return std::make_shared<TypeMatcher<long>>(value); }
IMatcher::Ptr Like(int value) { return std::make_shared<TypeMatcher<int>>(value); }
IMatcher::Ptr Like(double value) { return std::make_shared<TypeMatcher<double>>(value); }
IMatcher::Ptr Like(const std::string& value) {
    return std::make_shared<TypeMatcher<std::string>>(value);
}
IMatcher::Ptr Like(const char* value) { return std::make_shared<TypeMatcher<std::string>>(value); }
IMatcher::Ptr Like(bool value) { return std::make_shared<TypeMatcher<bool>>(value); }
...
IMatcher::Ptr Date(const std::string& format) { return std::make_shared<DateMatcher>(format); }
IMatcher::Ptr Time(const std::string& format, const std::string& example) {
    return std::make_shared<TimeMatcher>(format, example);
}
IMatcher::Ptr Time(const std::string& format) { return std::make_shared<TimeMatcher>(format); }
IMatcher::Ptr Matching(const std::string& regex, const std::string& example) {
    return std::make_shared<RegexMatcher>(regex, example);
}
IMatcher::Ptr Matching(const std::string& regex) { return std::make_shared<RegexMatcher>(regex); }
...
IMatcher::Ptr HexValue() { return std::make_shared<HexadecimalMatcher>(); }
IMatcher::Ptr IPAddress(const std::string& example) {
    return std::make_shared<IPAddressMatcher>(example);
}
IMatcher::Ptr IPAddress() { return std::make_shared<IPAddressMatcher>(); }
IMatcher::Ptr Numeric(int example) { return std::make_shared<NumberMatcher<int>>(example); }
IMatcher::Ptr Numeric(long example) { return std::make_shared<NumberMatcher<long>>(example); }
IMatcher::Ptr Numeric(float example) { return std::make_shared<NumberMatcher<float>>(example); }
IMatcher::Ptr Numeric(double example) { return std::make_shared<NumberMatcher<double>>(example); }
IMatcher::Ptr Numeric() { return std::make_shared<NumberMatcher<long>>(); }
IMatcher::Ptr Decimal(float example) { return std::make_shared<DecimalMatcher>(example); }
IMatcher::Ptr Decimal(double example) { return std::make_shared<DecimalMatcher>(example); }
IMatcher::Ptr Decimal() { return std::make_shared<DecimalMatcher>(); }
IMatcher::Ptr Uuid(const std::string& example) { return std::make_shared<UuidMatcher>(example); }
IMatcher::Ptr Uuid() { return std::make_shared<UuidMatcher>(); }
...
IMatcher::Ptr EqualTo(int value) { return std::make_shared<EqualsMatcher<int>>(value); }
IMatcher::Ptr EqualTo(long value) { return std::make_shared<EqualsMatcher<long>>(value); }
IMatcher::Ptr EqualTo(float value) { return std::make_shared<EqualsMatcher<float>>(value); }
IMatcher::Ptr EqualTo(double value) { return std::make_shared<EqualsMatcher<double>>(value); }
...
IMatcher::Ptr EqualTo(bool value) { return std::make_shared<EqualsMatcher<bool>>(value); }
IMatcher::Ptr IncludesStr(const std::string& value) {
    return std::make_shared<IncludesMatcher>(value);
}
IMatcher::Ptr NullValue() { return std::make_shared<NullMatcher>(); }
...
std::string RegexMatcher::getJson() const {
    ...
    if (!example.empty()) {
        if (pactffi_check_regex(regex.data(), example.data())) {
            ...
            stringStream << "regex: Example value '" << example
...
std::string EachlikeMatcher::getJson() const {
    if (min && min.value() > 0 && max && max.value() > 0 && min.value() > max.value()) {
        std::ostringstream stringStream;
        stringStream << "eachLike: Min array size " << min.value()
                     << " is greater than the max size " << max.value();
        BOOST_THROW_EXCEPTION(std::runtime_error(stringStream.str()));
    } else if (min && min.value() > 0 && examples < min.value()) {
        std::ostringstream stringStream;
        stringStream << "eachLike: Number of examples " << examples << " is less than the min size "
                     << min.value();
        BOOST_THROW_EXCEPTION(std::runtime_error(stringStream.str()));
    } else if (max && max.value() > 0 && examples > max.value()) {
        std::ostringstream stringStream;
        stringStream << "eachLike: Number of examples " << examples
                     << " is greater than the max size " << max.value();
        BOOST_THROW_EXCEPTION(std::runtime_error(stringStream.str()));
    }
...
std::string UrlMatcher::getJson() const {
    ...
    // Temporary fix for inconsistancies between matchers and generators. Matchers use "value"
    // attribute for example values, while generators use "example"
    j["value"] = example;
    if (basePath.empty()) {
        j["example"] = example;
